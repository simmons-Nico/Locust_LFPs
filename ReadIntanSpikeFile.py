...
"""Reads spike.dat files generated by Intan Technologies RHX data acquisition
software.  Data are parsed and placed into variables within the Python
workspace. Therefore, it is recommended to add either saving these variables
to disk or any plotting or processing of the data at the end of
readIntanSpikeFile, before those variables are removed when execution
completes.
...
def readIntanSpikeFile(option):
    """Reads an Intan 'spike.dat' file and, for demonstration purposes, plots
    a single snapshot from a specific spike on a specific channel. If the
    provided 'spike.dat' file does not include any snapshots, the last few
    lines of this function relating to snapshots should be commented out to
    ...
    if spikeFileVersionNumber > 1:
        print("Warning:")
        print("This spike file version is not supported by this file reader.")
        print("Check Intan Technologies website for a more recent version.")

    readString(fid)  # Returns filename, which is unused in this script.
    ...
    while filesize - fid.tell() > 0:
        ...
        if snapshotsPresent:
            snapshot = list(struct.unpack("<%dH" % nSamples,
                                          fid.read(2 * nSamples)))
        ...
        if snapshotsPresent:
            snapshotMicroVolts = [0.195 * (float(snapshotSample) - 32768.0)
                                  for snapshotSample in snapshot]
    ...
    if snapshotsPresent:
        tSnapshot = [(sample - samplesPreDetect) / sampleRate
                     for sample in range(nSamples)]

    # Just for demonstration, take the plot the 2nd (N = 1) channel's list of
    # snapshots (snapshots are always in the fifth column M = 4). Grab the 6th
    # snapshot present (list index = 5) and plot it.
...
class FileTypeError(Exception):
    """Exception return when file type is not recognized (magic number at the
    beginning is not consistent with an Intan spike file).
    """


if __name__ == '__main__':
    # If the function is called with the "noartifacts" parameter,
    # all spikes with spike ID = 128 are ignored.
